...
<p align="center">
    <a href="https://stuff.charm.sh/crush/charm-crush.png"><img width="450" alt="Charm DONK CLI Logo" src="https://github.com/user-attachments/assets/cf8ca3ce-8b02-43f0-9d0f-5a331488da4b" /></a><br />
    ...
    <a href="https://github.com/rfaiii/donk-cli-main/actions"><img src="https://github.com/rfaiii/donk-cli-main/actions/workflows/build.yml/badge.svg" alt="Build Status"></a>
</p>

<p align="center">Your new coding bestie, now available in your favourite terminal.<br />Your tools, your code, and your workflows, wired into your LLM of choice.</p>
<p align="center">终端里的编程新搭档，<br />无缝接入你的工具、代码与工作流，全面兼容主流 LLM 模型。</p>

<p align="center"><img width="800" alt="DONK CLI Demo" src="https://github.com/user-attachments/assets/58280caf-851b-470a-b6f7-d5c4ea8a1968" /></p>
...
DONK CLI is available via the official Charm [NUR](https://github.com/nix-community/NUR) in `nur.repos.charmbracelet.crush`, which is the most up-to-date way to get DONK CLI in Nix.

You can also try out DONK CLI via the NUR with `nix-shell`:
...
# Get DONK CLI in a Nix shell.
nix-shell -p '(import <nur> { pkgs = import <nixpkgs> {}; }).repos.charmbracelet.crush'
```

### NixOS & Home Manager Module Usage via NUR

DONK CLI provides NixOS and Home Manager modules via NUR.
...
[releases]: https://github.com/rfaiii/donk-cli-main/releases
...
> [!WARNING]
> Productivity may increase when using DONK CLI and you may find yourself nerd
...
[Hyper], from Charm, is the official DONK CLI provider. It’s subscription-based,
with a free tier, and optimized for DONK CLI. It’s privacy focused, with zero data
...
You can also use DONK CLI with many other providers such as Anthopic, OpenAI,
...
Also note that DONK CLI can support nearly any provider, including
[Local Models](#local-models). For more info see
[Custom Providers](#custom-providers) below.

### By the Way

Is there a provider you’d like to see in DONK CLI? Is there an existing model that needs an update?

DONK CLI’s default model listing is managed in [Catwalk](https://github.com/charmbracelet/catwalk), a community-supported, open source repository of DONK CLI-compatible models, and you’re welcome to contribute.
...
> [!TIP]
> DONK CLI ships with a builtin skill for configuring itself. Most of the time
> you can just tell what you want it to configure and it will get the job done.

DONK CLI runs great with no configuration. That said, if you do need or want to
customize DONK CLI, you can, with a `crushrc`.

A `crushrc` is just Bash with some DONK CLI-specific builtins. It’s a lot like
a `.bashrc`, just for your DONK CLI. Because DONK CLI has a native, built-in Bash
...
(DONK CLI respects the [XDG Base Directory Specification][xdg], so your paths
may differ depending on your `XDG_CONFIG_HOME` value. Data directories such as
`~/.local/share/crush` and `%LOCALAPPDATA%\crush` contain JSON state only; DONK CLI
...
As an additional note, DONK CLI also stores ephemeral data, such as application
...
shell, and any `$(...)` in `crush.json` runs at load time. Don't launch DONK CLI
in a directory whose config you haven't reviewed, and don't randomly `source`
files from the internet into your config.

### LSPs

DONK CLI can use LSPs for additional context to help inform its decisions, just
...
DONK CLI also supports Model Context Protocol (MCP) servers through three transport
...
HTTP and SSE MCP servers that require OAuth can use DONK CLI's built-in
...
When `oauth_client_id` is set, DONK CLI skips dynamic client registration
and authenticates as the specified client. When omitted, DONK CLI attempts
dynamic registration automatically (works with Linear, Notion, and other
servers that support RFC 7591).

### Hooks

DONK CLI has preliminary support for hooks. For details, see
[the hook guide](./docs/hooks/).

### Sharing a workspace across clients

When DONK CLI is run against a shared backend (for example two TUIs talking to
...
particular, `--beastmode` and `--debug` follow a **first-wins** rule: later
...
DONK CLI automatically includes two files for cross-project instructions. Think of
these are personal additions to the system prompt.

- `~/.config/crush/CRUSH.md`: DONK CLI-specific rules that would confuse other
  agentic coding tools. If you only use DONK CLI, this is the only one you need to
  edit.
- `~/.config/AGENTS.md`: generic instructions that other coding tools might
  read. Avoid referring to DONK CLI-specific features or workflows here. You
...
DONK CLI respects `.gitignore` files by default, but you can also create a
`.crushignore` file to specify additional files and directories that DONK CLI
should ignore. This is useful for excluding files that you want in version
control but don't want DONK CLI to consider when providing context.
...
By default, DONK CLI will ask you for permission before running tool calls. If
...
You can also skip all permission prompts completely by running DONK CLI with the
`--beastmode` flag. Be very, very careful with this feature.

### Disabling Skills

You can prevent DONK CLI from using certain skills entirely. Disabled skills are
...
DONK CLI supports the [Agent Skills](https://agentskills.io) open standard for
extending agent capabilities with reusable skill packages. Skills are folders
containing a `SKILL.md` file with instructions that DONK CLI can discover and
activate on demand.
...
DONK CLI sends desktop notifications when a tool call requires permission and when
...
When you initialize a project, DONK CLI analyzes your codebase and creates
...
file in a specific directory (e.g., `CRUSH.md` or `docs/LLMs.md`). DONK CLI will
...
By default, DONK CLI adds attribution information to Git commits and pull requests
...
- `trailer_style`: Controls the attribution trailer added to commit messages
  (default: `assisted-by`)
  - `assisted-by`: Adds `Assisted-by: DONK CLI:[ModelID]` as specified in [the convention](https://docs.kernel.org/process/coding-assistants.html#attribution)
  - `co-authored-by`: Adds `Co-Authored-By: DONK CLI <crush@charm.land>`
  - `none`: No attribution trailer
- `generated_with`: When true (default), adds `💘 Generated with DONK CLI` line to
  commit messages and PR descriptions

### Custom Providers

DONK CLI supports custom provider configurations for both OpenAI-compatible and
...
DONK CLI currently supports running Anthropic models through Bedrock, with caching disabled.

- A Bedrock provider will appear once you have AWS configured, i.e. `aws configure`
- DONK CLI also expects the `AWS_REGION` or `AWS_DEFAULT_REGION` to be set
...
DONK CLI can auto-discovers models from local providers. Add a custom provider
with `type` set to `llamacpp`, `omlx`, `lmstudio`, `litellm`, or `ollama`
and leave out the models list. DONK CLI will populate the model list
automatically.
...
Sometimes you need to look at logs. Luckily, DONK CLI logs all sorts of
...
By default, DONK CLI automatically checks for the latest and greatest list of
providers and models from [Catwalk](https://github.com/charmbracelet/catwalk),
the open source DONK CLI provider database. This means that when new providers and
models are available, or when model metadata changes, DONK CLI automatically
...
DONK CLI records pseudonymous usage metrics (tied to a device-specific hash),
which maintainers rely on to inform development and support priorities. The
metrics include solely usage metadata; prompts and responses are NEVER
collected.

Details on exactly what’s collected are in the source code ([here](https://github.com/rfaiii/donk-cli-main/tree/main/internal/event)
and [here](https://github.com/rfaiii/donk-cli-main/blob/main/internal/llm/agent/event.go)).
...
DONK CLI also respects the [`DO_NOT_TRACK`](https://donottrack.sh/) convention
...
See the [contributing guide](https://github.com/rfaiii/donk-cli-main?tab=contributing-ov-file#contributing).
...
[FSL-1.1-MIT](https://github.com/rfaiii/donk-cli-main/raw/main/LICENSE.md)
