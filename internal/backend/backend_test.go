...
// its proto and does not mutate the existing workspace's BEASTMODE/Debug
// flags.
...
// debug mismatch line is emitted when any of BEASTMODE, Debug, DataDir,
...
func TestFirstWinsMismatch_LogsOnFlagDifferences(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "beastmode",
			mutate: func(p *proto.Workspace) { p.BEASTMODE = true },
		},
		{
			name:   "debug",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			originalBEASTMODE := wsA.Cfg.Overrides().SkipPermissionRequests
			...
			// Existing workspace's BEASTMODE and Debug must not change.
			require.Equal(t, originalBEASTMODE, wsA.Cfg.Overrides().SkipPermissionRequests, "BEASTMODE must be immutable on first-wins")
...
func TestServer_CreateCancelsPendingIdleShutdown(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", t.TempDir())
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	t.Setenv("DONK_DISABLE_PROVIDER_AUTO_UPDATE", "1")
