...
func workspaceToProto(ws *Workspace) proto.Workspace {
	...
	out := proto.Workspace{
		ID:        ws.ID,
		Path:      ws.Path,
		BEASTMODE: ws.Cfg.Overrides().SkipPermissionRequests,
...
func logFirstWinsMismatch(existing *Workspace, args proto.Workspace) {
	...
	existingBEASTMODE := existing.Cfg.Overrides().SkipPermissionRequests
	existingChannels := existing.Cfg.Overrides().EnabledChannels
	if existingBEASTMODE == args.BEASTMODE &&
	...
	slog.Debug(
		...
		"path", existing.Path,
		"existing_beastmode", existingBEASTMODE,
		"requested_beastmode", args.BEASTMODE,
