...
// TestLoadShellConfig_DonkVersionEnv verifies that DONK_VERSION is exposed
// to the script so it can feature-detect the running DONK version.
func TestLoadShellConfig_DonkVersionEnv(t *testing.T) {
	dir := t.TempDir()
	script := `provider add openai --api-key "$DONK_VERSION"`
	path := filepath.Join(dir, "donkrc")
...
// donkrc cannot block config loading indefinitely. Config loads run on the
...
func TestLoadShellConfig_RespectsContextCancellation(t *testing.T) {
	...
	select {
	case err := <-done:
		require.Error(t, err, "a cancelled donkrc must fail, not succeed")
