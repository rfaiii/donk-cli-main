...
// TestLoadShellConfig_BvrVersionEnv verifies that BVR_VERSION is exposed
// to the script so it can feature-detect the running BVR version.
func TestLoadShellConfig_BvrVersionEnv(t *testing.T) {
	dir := t.TempDir()
	script := `provider add openai --api-key "$BVR_VERSION"`
	path := filepath.Join(dir, "bvrrc")
...
// bvrrc cannot block config loading indefinitely. Config loads run on the
...
func TestLoadShellConfig_RespectsContextCancellation(t *testing.T) {
	...
	select {
	case err := <-done:
		require.Error(t, err, "a cancelled bvrrc must fail, not succeed")
