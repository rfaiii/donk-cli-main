---
name: bvr-config
description: Use when the user needs help configuring BVR — writing bvrrc (the Bash config format) or bvr.json, setting up providers, models, LSPs, MCP servers, hooks, skills, permissions, or changing BVR behavior.
---

# BVR Configuration

BVR supports two config formats:

- **`bvrrc`** — a Bash script that builds config by calling BVR builtins.
  **Preferred.** Because it is real Bash you get includes, secrets,
  conditionals, and variables for free.
- **`bvr.json`** — static JSON. Fully supported; see
  [Legacy JSON format](#legacy-json-format).

Both are discovered together and deep-merged. Priority (highest to lowest):

1. `.bvrrc` / `bvrrc` / `.bvr.json` / `bvr.json` (project-local,
   closer-to-cwd wins; Windows uses `.\.bvrrc` / `.\bvrrc`)
2. `$XDG_CONFIG_HOME/bvr/bvrrc` or `~/.config/bvr/bvrrc`
   (`%XDG_CONFIG_HOME%\bvr\bvrrc` or
   `%USERPROFILE%\.config\bvr\bvrrc` on Windows)

Data directories (`~/.local/share/bvr` and `%LOCALAPPDATA%\bvr`) contain
machine-owned JSON state only; BVR does not discover or execute a `bvrrc`
from those locations.

If a directory has both `bvrrc` and `bvr.json`, they merge (`bvrrc` wins
on conflicts) and BVR logs a warning.

## bvrrc at a glance

A `bvrrc` is a plain Bash script executed at load time with the same embedded
...
`BVR_VERSION` is exported into the script so you can feature-detect the
running BVR (it is the literal `devel` for local builds):

```bash
[[ "$BVR_VERSION" != devel ]] && lsp add gopls --command gopls
```
...
- `<provider>/<id>` is the same form `bvr-cli models` prints. A missing slash is
...
```bash
hook add PreToolUse --matcher "^bash$" --command ".bvr/hooks/no-haskell.sh" --name no-haskell
```
...
option skill-path ./skills
option disable-skill bvr-config
...
> `skill-path`: `.agents/skills`, `.bvr/skills`, `.claude/skills`,
> `.cursor/skills`.
...
| ---------------------------- | ------------------------------------------------- |
| `BVR_EVENT`                | Event name (e.g. `PreToolUse`)                    |
| `BVR_TOOL_NAME`            | Name of the tool being called                     |
| `BVR_SESSION_ID`           | Current session ID                                |
| `BVR_CWD`                  | Current working directory                         |
| `BVR_PROJECT_DIR`          | Project root directory                            |
| `BVR_TOOL_INPUT_COMMAND`   | Value of `command` from tool input (if present)   |
| `BVR_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
BVR also accepts the Claude Code hook output format, so existing hooks work
unchanged:
...
- `BVR_VERSION` — exported into `bvrrc` at load; the running version (or
  `devel` for local builds).
- `BVR_GLOBAL_CONFIG` — override global config location.
- `BVR_GLOBAL_DATA` — override data directory location.
- `BVR_SKILLS_DIR` — override default skills directory.

## Legacy JSON format

`bvr.json` is the original static format. It still works and merges with
`bvrrc`. Basic structure:

```json
{
  "$schema": "https://charm.land/bvr.json",
  "models": {},
  "providers": {},
  "mcp": {},
  "lsp": {},
...
### Shell expansion in bvr.json

In JSON, only selected string fields are run through the embedded shell at load
time (in `bvrrc`, everything is native Bash so this table does not apply):
...
Both formats are trusted code. `bvrrc` runs entirely, and any `$(...)` in
`bvr.json` runs at load time, with the invoking user's shell privileges,
before the UI appears. Don't launch BVR in a directory whose config you
haven't reviewed.
