...
func (w *countingWorkspace) PermissionSkipRequests() bool { w.permCalls++; return w.beastmode }
...
// busy/beastmode getters used by render paths serve the memoized value and never
...
// TestToggleBeastmodeWritesThroughCache: both beastmode toggle paths share
// toggleBeastmodeMode, which must write the known new value through the cache —
// no invalidation, no re-probe.
func TestToggleBeastmodeWritesThroughCache(t *testing.T) {
	pinTTLs(t)

	ws := &countingWorkspace{ready: true, beastmode: false}
	m := newBusyUI(ws)

	got := m.toggleBeastmodeMode()
	require.True(t, got)
	...
	require.True(t, m.beastmodeModeCached(), "the new value must be served from the cache")
	require.True(t, m.beastmodeCache.fresh(busyCacheTTL), "write-through must stamp the cache fresh")
	m.beastmodeModeCached()
	require.Equal(t, readsAfterToggle, ws.permCalls, "reads after the toggle must not re-probe")

	got = m.toggleBeastmodeMode()
	require.False(t, got)
	require.False(t, m.beastmodeModeCached())
}

// TestLocalBeastmodeToggleSupersedesInFlightProbe pins the generation bump in
// toggleBeastmodeMode: a busy/beastmode probe dispatched before the toggle carries the
...
func TestLocalBeastmodeToggleSupersedesInFlightProbe(t *testing.T) {
	pinTTLs(t)
	...
	// A busy/beastmode probe carrying the pre-toggle generation is in flight.
	m.busyFetchInFlight = true
	staleGen := m.busyFetchGen

	require.True(t, m.toggleBeastmodeMode())
	require.NotEqual(t, staleGen, m.busyFetchGen,
		"toggle must advance the busy generation to supersede in-flight probes")
	require.True(t, m.beastmodeModeCached(), "toggle must write the new value through the cache")

	// The stale probe (old generation, old beastmode=false) lands.
	m.busyFetchInFlight = true
	cmds := m.applyBusyState(busyStateMsg{gen: staleGen, beastmode: false})
	require.True(t, m.beastmodeModeCached(),
...
// TestRemoteBeastmodeToggleUpdatesEditorPrompt pins the second fix: when an
// asynchronous busy-state refresh reports a beastmode different from the
...
func TestRemoteBeastmodeToggleUpdatesEditorPrompt(t *testing.T) {
	pinTTLs(t)
	...
	// A remote toggle flips beastmode on; delivered via an off-thread refresh.
	m.applyBusyState(busyStateMsg{gen: m.busyFetchGen, beastmode: true})
	require.True(t, m.beastmodeModeCached(), "the refresh must write the new beastmode value through the cache")
	beastmodePrompt := ansi.Strip(m.textarea.View())
	require.NotEqual(t, normalPrompt, beastmodePrompt,
		"a remote beastmode toggle must change the rendered editor prompt")
	require.Contains(t, beastmodePrompt, "Y",
		"the beastmode prompt icon must render after a remote toggle")

	// Flipping back off must restore the normal prompt.
	m.applyBusyState(busyStateMsg{gen: m.busyFetchGen, beastmode: false})
	require.False(t, m.beastmodeModeCached())
	require.Equal(t, normalPrompt, ansi.Strip(m.textarea.View()),
		"toggling beastmode off must restore the normal editor prompt")
}
