...
//
//   - Reads (isAgentBusy, beastmodeModeCached, promptQueue) always return the
//     memoized value, stale or not.
//   - State edges (message created, agent finished/errored, prompt
//     submitted, cancel, session switch, beastmode toggle) invalidate or
...
func (m *UI) dispatchBusyRefresh() tea.Cmd {
	...
	return func() tea.Msg {
		...
		st.beastmode = ws.PermissionSkipRequests()
		return st
	}
}
...
func (m *UI) applyBusyState(msg busyStateMsg) []tea.Cmd {
	...
	prevBeastmode := m.beastmodeModeCached()
	m.agentBusyCache.set(msg.agentBusy)
	m.beastmodeCache.set(msg.beastmode)
	if prevBeastmode != msg.beastmode {
		// A remote/async toggle changed beastmode: update the editor
		...
		// the Update tail.
		m.setEditorPrompt(msg.beastmode)
	}
...
func (m *UI) staleWorkspaceRefreshCmds() []tea.Cmd {
	...
	var cmds []tea.Cmd
	if !m.agentBusyCache.fresh(busyCacheTTL) || !m.beastmodeCache.fresh(busyCacheTTL) {
		if cmd := m.dispatchBusyRefresh(); cmd != nil {
...
// toggleBeastmodeMode flips permission auto-approval and writes the new value
// through the beastmode cache (no re-probe needed) and the editor prompt. Shared
// by the direct keybinding and the commands-dialog action so both stay
// write-through. Returns the new mode.
func (m *UI) toggleBeastmodeMode() bool {
	beastmode := !m.com.Workspace.PermissionSkipRequests()
	m.com.Workspace.PermissionSetSkipRequests(beastmode)
	m.beastmodeCache.set(beastmode)
	// Supersede any in-flight busy/beastmode probe: its result carries the old
	...
	m.busyFetchGen++
	m.setEditorPrompt(beastmode)
	return beastmode
}

// beastmodeModeCached reports the memoized permission-skip ("beastmode") mode. Toggles
// write through the cache; the Update-tail backstop keeps it bounded-stale
// otherwise.
func (m *UI) beastmodeModeCached() bool {
	return m.beastmodeCache.val
}
