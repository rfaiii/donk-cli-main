...
// config file at path, mimicking what another bvr instance would leave
...
// which lets a single test process faithfully simulate two bvr instances:
...
func TestRefreshOAuthToken_InProcessSingleFlight(t *testing.T) {
	...
	configPath := filepath.Join(t.TempDir(), "bvr.json")
