...
// loadBvrSh writes a bvr.sh into an isolated project and loads it through
...
func loadBvrSh(t *testing.T, script string) *config.ConfigStore {
	t.Helper()
	store, err := loadBvrShErr(t, script)
	require.NoError(t, err)
	return store
}

// loadBvrShErr is loadBvrSh without asserting success, for cases that are
// expected to fail at load time.
func loadBvrShErr(t *testing.T, script string) (*config.ConfigStore, error) {
	t.Helper()
	...
	workDir := t.TempDir()
	dataDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(workDir, "bvrrc"), []byte(script), 0o644))

	return config.Load(workDir, dataDir, false)
}

func TestShellConfigPermissionsAllow(t *testing.T) {
	store := loadBvrSh(t, `permissions allow bash view`)
...
func TestShellConfigPermissionsAccumulateAndDedup(t *testing.T) {
	store := loadBvrSh(t, `permissions allow bash
...
func TestShellConfigPermissionsDeny(t *testing.T) {
	store := loadBvrSh(t, `permissions deny bash sourcegraph
