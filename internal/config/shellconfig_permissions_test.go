...
// loadDonkSh writes a donk.sh into an isolated project and loads it through
...
func loadDonkSh(t *testing.T, script string) *config.ConfigStore {
	t.Helper()
	store, err := loadDonkShErr(t, script)
	require.NoError(t, err)
	return store
}

// loadDonkShErr is loadDonkSh without asserting success, for cases that are
// expected to fail at load time.
func loadDonkShErr(t *testing.T, script string) (*config.ConfigStore, error) {
	t.Helper()
	...
	workDir := t.TempDir()
	dataDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(workDir, "donkrc"), []byte(script), 0o644))

	return config.Load(workDir, dataDir, false)
}

func TestShellConfigPermissionsAllow(t *testing.T) {
	store := loadDonkSh(t, `permissions allow bash view`)
...
func TestShellConfigPermissionsAccumulateAndDedup(t *testing.T) {
	store := loadDonkSh(t, `permissions allow bash
...
func TestShellConfigPermissionsDeny(t *testing.T) {
	store := loadDonkSh(t, `permissions deny bash sourcegraph
