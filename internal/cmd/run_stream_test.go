...
// with reason=tool_use and used to terminate `donk-cli run` early via
...
// subscribers (e.g. a TUI watching workspace events while `donk
...
// non-nil error from `donk-cli run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `donk-cli run` waits) exits cleanly rather
...
// queue scenario: `donk-cli run --continue` attaches to a session
