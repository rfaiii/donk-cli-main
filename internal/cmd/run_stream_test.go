...
// with reason=tool_use and used to terminate `bvr-cli run` early via
...
// subscribers (e.g. a TUI watching workspace events while `bvr
...
// non-nil error from `bvr-cli run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `bvr-cli run` waits) exits cleanly rather
...
// queue scenario: `bvr-cli run --continue` attaches to a session
