...
// UpdateAvailable is published when a newer BVR release is detected
...
// completion contract (notably `bvr-cli run` in client/server mode)
...
// `bvr-cli run`, which may attach to a busy session whose currently
