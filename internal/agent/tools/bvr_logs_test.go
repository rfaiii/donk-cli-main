...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "bvr.log")
...
func TestNewBvrLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewBvrLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, BvrLogsToolName, tool.Info().Name)
}

func TestBvrLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runBvrLogs(logFile, BvrLogsParams{Lines: 3})
...
func TestBvrLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runBvrLogs(logFile, BvrLogsParams{Lines: 0})
...
func TestBvrLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runBvrLogs(logFile, BvrLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestBvrLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runBvrLogs("/nonexistent/path/bvr.log", BvrLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestBvrLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "bvr.log")
	...
	result := runBvrLogs(logFile, BvrLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestBvrLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "bvr.log")
	...
	result := runBvrLogs(logFile, BvrLogsParams{Lines: 10})
...
func TestBvrLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runBvrLogs(logFile, BvrLogsParams{Lines: 1})
...
func TestBvrLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestBvrLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestBvrLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestBvrLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "bvr.log")
...
func TestBvrLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "bvr.log")
...
func TestBvrLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestBvrLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestBvrLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestBvrLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runBvrLogs(logFile, BvrLogsParams{Lines: 5})
...
func TestBvrLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
