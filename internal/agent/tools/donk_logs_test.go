...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "donk.log")
...
func TestNewDonkLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewDonkLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, DonkLogsToolName, tool.Info().Name)
}

func TestDonkLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runDonkLogs(logFile, DonkLogsParams{Lines: 3})
...
func TestDonkLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runDonkLogs(logFile, DonkLogsParams{Lines: 0})
...
func TestDonkLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runDonkLogs(logFile, DonkLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestDonkLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runDonkLogs("/nonexistent/path/donk.log", DonkLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestDonkLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "donk.log")
	...
	result := runDonkLogs(logFile, DonkLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestDonkLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "donk.log")
	...
	result := runDonkLogs(logFile, DonkLogsParams{Lines: 10})
...
func TestDonkLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runDonkLogs(logFile, DonkLogsParams{Lines: 1})
...
func TestDonkLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestDonkLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestDonkLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestDonkLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "donk.log")
...
func TestDonkLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "donk.log")
...
func TestDonkLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestDonkLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestDonkLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestDonkLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runDonkLogs(logFile, DonkLogsParams{Lines: 5})
...
func TestDonkLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
