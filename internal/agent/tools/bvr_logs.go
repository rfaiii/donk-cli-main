...
const BvrLogsToolName = "bvr_logs"

//go:embed bvr_logs.md.tpl
var bvrLogsDescriptionTmpl []byte

var bvrLogsDescriptionTpl = template.Must(
	template.New("bvrLogsDescription").
		Parse(string(bvrLogsDescriptionTmpl)),
)

type bvrLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func bvrLogsDescription() string {
	return renderTemplate(bvrLogsDescriptionTpl, bvrLogsDescriptionData{
...
type BvrLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewBvrLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		BvrLogsToolName,
		bvrLogsDescription(),
		func(ctx context.Context, params BvrLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runBvrLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runBvrLogs reads and formats the last N log entries from the given file.
func runBvrLogs(logFile string, params BvrLogsParams) string {
