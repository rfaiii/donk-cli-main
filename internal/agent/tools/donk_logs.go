...
const DonkLogsToolName = "donk_logs"

//go:embed donk_logs.md.tpl
var donkLogsDescriptionTmpl []byte

var donkLogsDescriptionTpl = template.Must(
	template.New("donkLogsDescription").
		Parse(string(donkLogsDescriptionTmpl)),
)

type donkLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func donkLogsDescription() string {
	return renderTemplate(donkLogsDescriptionTpl, donkLogsDescriptionData{
...
type DonkLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewDonkLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		DonkLogsToolName,
		donkLogsDescription(),
		func(ctx context.Context, params DonkLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runDonkLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runDonkLogs reads and formats the last N log entries from the given file.
func runDonkLogs(logFile string, params DonkLogsParams) string {
