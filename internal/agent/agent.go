...
func NewSessionAgent(
	...
	return &sessionAgent{
		...
		isBeastmode:          opts.IsBeastmode,
		isCoderAgent:         opts.IsCoderAgent,
...
// IsCoderAgent reports whether this session agent was created for the coder role.
func (a *sessionAgent) IsCoderAgent() bool {
	return a.isCoderAgent
}
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	result, err = agent.Stream(genCtx, fantasy.AgentStreamCall{
		...
		OnStepFinish: func(stepResult fantasy.StepResult) error {
			...
			if a.IsCoderAgent() {
				hasToolCalls := len(stepResult.Content.ToolCalls()) > 0
				hasText := len(stepResult.Content.Text()) > 0
				slog.Info("Coder agent step finished", "finish_reason", string(stepResult.FinishReason), "tool_calls", hasToolCalls, "text", hasText)
			}
