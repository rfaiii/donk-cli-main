# donk-cli: Multi-LLM Provider Integration Spec

## Context
This document serves as an implementation blueprint for `donk-cli`. The objective is to refactor the application to support multiple LLM providers (Ollama, OpenRouter, Gemini) using the Strategy Pattern, driven by a local JSON configuration file. 

## Target Architecture
- **Language:** Go
- **Configuration Path:** OS-agnostic user config directory (e.g., `~/.config/donk/config.json` on macOS/Linux).
...
- Implement a `LoadConfig()` function:
  ...
  - Read and parse `donk/config.json`.
